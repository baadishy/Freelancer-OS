...
export async function analyzeOpportunity(
  ...
  if (op.validationStatus !== 'VALID') {
    db.addLog('warning', 'gemini', `[AI REFUSED] Refused to analyze opportunity "${op.title}" (${op.id}). Status: ${op.validationStatus || 'unvalidated'}`);
    throw new Error(`AI Match Analysis Refused: Target opportunity does not satisfy health eligibility. Needs status 'VALID'.`);
  }
...
export async function writeProposal(
  ...
  if (op.validationStatus !== 'VALID') {
    db.addLog('warning', 'automation', `[PROPOSAL REFUSED] Locked proposal generation for invalid opportunity "${op.title}" (${op.id}). Status: ${op.validationStatus || 'unvalidated'}`);
